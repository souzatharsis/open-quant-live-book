...
x2 <- x2[-1]
```
```{r rand, echo=FALSE, out.width='100%', fig.align = "center", fig.cap="Life is Random (or Nonlinear?)"}
...
In this Chapter, we will introduce a prediction-based definition of causality and its implementation using a vector auto-regression formulation. Next, we will introduce a probabilistic-definition of causality and its implementation using an information-theoretical framework. We will then simulate linear and nonlinear systems and uncover causal links with the proposed methods. In the final section, we will quantify information flow among global equity indexes further uncovering which indexes and markets are driving the global financial markets. Finally, we will discuss further applications including the impact of social media sentiment in financial and crypto markets.
...
We use the function `calc_te` from the package **RTransferEntropy** and the previously defined function `Linear.GC` to calculate pairwise information flow among the simulated variables as follows:
...
```{r causality-graph-GC, echo=FALSE, out.width='100%', fig.align = "center",fig.cap="Granger-Causality of simulated linear system"}
...
```{r causality-graph-TE, echo=FALSE, out.width='100%', fig.align = "center",fig.cap="Transfer Entropy of simulated linear system"}
...
```{r causality-graph-nonlinear-GC, echo=FALSE, out.width='100%', fig.align = "center",fig.cap="Granger-Causality of simulated nonlinear system"}
...
```{r causality-graph-nonlinear-TE, echo=FALSE, out.width='100%', fig.align = "center",fig.cap="Transfer Entropy of simulated nonlinear system"}
...
```{r te-crypto, echo=FALSE, out.width='100%', fig.align = "center", fig.cap="Evidence that BTC sentiment and price are causally coupled in both directions in a non-linear way. Non-linear TE is calculated by multidimensional histograms with 6 quantile bins per dimension. Z-scores, calculated over 50 shuffles, show a high level of significance, especially during 2017 and 2018, in both directions."}
